...
function whenLabel(iso:string,tz:string){
  try{return new Intl.DateTimeFormat("en-US",{weekday:"long",month:"long",day:"numeric",hour:"numeric",minute:"2-digit",timeZone:tz||"America/New_York",timeZoneName:"short"}).format(new Date(iso));}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const {data:appointments,error:apptError}=await admin.from("booking_appointments")
      .select("id,card_id,service_name,customer_name,customer_email,start_at,timezone,manage_token,reminder_24h_sent_at,reminder_2h_sent_at")
      .eq("kind","booking").eq("status","confirmed").eq("source_environment",environment)
      .not("customer_email","is",null).gt("start_at",now.toISOString()).lte("start_at",horizon)
      .order("start_at").limit(250);
    ...
    for(const appt of appointments){
      ...
      const hours=(startMs-now.getTime())/3600000;
      ...
      const {data:claim,error:claimError}=await admin.from("booking_appointments")
        .update({[field]:claimedAt}).eq("id",appt.id).eq("status","confirmed").is(field,null).select("id").maybeSingle();
      ...
      const response=await fetch("https://api.resend.com/emails",{
        ...
        body:JSON.stringify({from,to:[email],subject:`Reminder: ${service} — ${business}`,html,reply_to:validEmail(replyTo)?replyTo:undefined}),
      });
      if(response.ok){sent++;}
      else{
        failed++;
        await admin.from("booking_appointments").update({[field]:null}).eq("id",appt.id).eq(field,claimedAt);
        console.error("booking-reminders-v2",appt.id,await response.text().catch(()=>"Resend error"));
      }
    }
    return json({ok:true,environment,checked:appointments.length,sent,failed,skipped});
  }catch(error){
    console.error("booking-reminders-v2",error);
    return json({error:error instanceof Error?error.message:"Unable to run reminders"},500);
  }
});
