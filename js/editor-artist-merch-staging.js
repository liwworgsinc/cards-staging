/* LIW Cards staging — Artist Store compatibility bridge.
   The Artist Control Center mounts the native LIW Product Showcase directly
   inside its Store tab. */
(function(){
  'use strict';
  ...
  function sync(){
    ...
    const output=room.querySelector('[data-artist-count="store"]');if(output)output.textContent=String(productCount());return true;
  }
  document.addEventListener('click',event=>{if(event.target.closest?.('#add-product,[data-remove-product]'))setTimeout(sync,120);},true);
  document.addEventListener('change',event=>{if(event.target.closest?.('#product-list,[name="products_enabled"]'))setTimeout(sync,120);},true);
  let tries=0;const timer=setInterval(()=>{tries++;if(sync()&&tries>12)clearInterval(timer);if(tries>80)clearInterval(timer);},250);sync();
})();
...
/* Mobile-safe performer module. No document-wide MutationObserver. */
(function loadArtistPerformerModulesV2(){
  if(document.querySelector('script[data-liw-artist-performer-modules-v2]'))return;
  const script=document.createElement('script');
  script.src='js/editor-artist-performer-modules-v2-staging.js?v=20260907-mobilefix-1';
  script.defer=true;
  script.dataset.liwArtistPerformerModulesV2='true';
