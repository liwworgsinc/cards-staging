...
(function(){
  ...
  function stopObserver(){try{observer?.disconnect();}catch(_){ }observer=null;if(observerStop){clearTimeout(observerStop);observerStop=0;}}
    ...
    if(!node){
      ...
      node.innerHTML=`
        ...
        <div class="barber-template-preview">
          ...
          <div class="barber-template-meta"><div><span>Font</span><strong data-barber-template-font>DM Sans</strong></div><div><span>Buttons</span><strong data-barber-template-button-style>Filled</strong></div></div>
        </div>
  ...
  function restoreBarberAfterTemplate(){
    ...
    const node=q('[data-barber-template-bridge]');if(node)render(node);
  }

  function reapplySelectedTemplate(templateId){
    if(templateId){const template=templateList().find(item=>String(item.id)===String(templateId));if(template&&typeof applyTemplate==='function')try{applyTemplate(template);}catch(_){ }}
  ...
  window.addEventListener('click',event=>{
    ...
    if(barberChoice){const templateId=val('template_id','');setTimeout(()=>reapplySelectedTemplate(templateId),0);return;}
    const templateButton=target.closest('.template-card');if(templateButton&&barberActive())setTimeout(()=>restoreBarberAfterTemplate(),0);
  },true);
  document.addEventListener('input',event=>{if(event.target?.matches?.('[name="primary_color"],[name="secondary_color"],[name="background_color"],[name="font_family"],[name="button_style"]'))setTimeout(()=>render(q('[data-barber-template-bridge]')),0);},true);
  document.addEventListener('click',event=>{if(event.target?.closest?.('.template-card'))setTimeout(()=>render(q('[data-barber-template-bridge]')),120);},true);

  function start(){
    if(mount())return;
    const target=q('.editor-panel[data-panel="design"]')||document.body;if(!target)return;
    observer=new MutationObserver(()=>mount());observer.observe(target,{childList:true,subtree:true});observerStop=setTimeout(stopObserver,4500);
  }
...
(function(){
  ...
  function injectStyles(){
    ...
    const style=document.createElement('style');style.id='liw-studio-business-type-style';
    style.textContent=`
  ...
  function updateExperienceCard(){
    ...
    const note=q('.card-experience-note',section||document);
    if(note)note.innerHTML='<strong>Your content stays yours:</strong> Classic, Flow, Showtime and Studio all use the same saved LIW card details. Studio opens its own adaptive Control Center below.';
    const saveCopy=q('.card-experience-save-copy span',section||document);
    if(saveCopy)saveCopy.textContent='Save Classic, Flow, Showtime or Studio immediately without scrolling back to the top.';
  }

  function updateControlCenterCopy(){
    const center=q('#barber-control-center');if(!center)return;
    const meta=TYPES[selectedType]||TYPES.barber;
    const heroIcon=q('.barber-v5-hero-icon',center);if(heroIcon)heroIcon.innerHTML=businessIcon(selectedType,21);
    const heroEyebrow=q('.barber-v5-hero-copy > span',center);if(heroEyebrow)heroEyebrow.textContent='LIW STUDIO EXPERIENCE';
    const heroTitle=q('.barber-v5-hero-copy h3',center);if(heroTitle)heroTitle.textContent='Studio Control Center';
    const heroCopy=q('.barber-v5-hero-copy p',center);if(heroCopy)heroCopy.textContent=`${meta.label} mode is active. Your saved card details, client promo and Studio tools stay together here.`;
    const tabs=q('.barber-v5-tabs',center);if(tabs)tabs.setAttribute('aria-label','Studio Control Center');
    const infoTab=q('[data-barber-v5-tab="info"]',center);if(infoTab)infoTab.innerHTML=`${businessIcon(selectedType,15)} ${meta.label} Info`;
    const infoHead=q('[data-barber-v5-panel="info"] .barber-v5-panel-head strong',center);if(infoHead)infoHead.textContent='Your Studio profile';
    const infoSub=q('[data-barber-v5-panel="info"] .barber-v5-panel-head span',center);if(infoSub)infoSub.textContent='These are the same saved details used by your public card.';
    const labels={barber_name:'Display name',specialty:'Specialty / title',shop_name:'Business / studio name',location:'Business location',phone:'Call number',text_phone:'Text number',promo:'Promo / availability',welcome:'Welcome message'};
    Object.entries(labels).forEach(([field,label])=>{const input=q(`[data-barber-v5-field="${field}"]`,center);const span=input?.closest('label')?.querySelector(':scope > span');if(span)span.textContent=label;});
    const note=q('[data-barber-v5-panel="info"] .barber-v5-note span',center);if(note)note.textContent='Studio uses the same saved LIW card information. Changing the business type only adapts the presentation—it does not duplicate or erase your data.';
  }

  function renderPreviewBadge(){
    const cover=q('#preview-cover');if(!cover)return;
    let badge=q('.studio-preview-industry',cover);
    if(!studioActive()){badge?.remove();delete document.body.dataset.studioBusinessType;return;}
    if(!badge){badge=document.createElement('div');badge.className='studio-preview-industry';cover.appendChild(badge);}
    const meta=TYPES[selectedType]||TYPES.barber;badge.innerHTML=`${businessIcon(selectedType,16)}<span>${meta.label}</span>`;document.body.dataset.studioBusinessType=selectedType;
  }

  function syncButtons(){
    qa('[data-studio-business-type]').forEach(button=>{const active=button.dataset.studioBusinessType===selectedType;button.classList.toggle('is-active',active);button.setAttribute('aria-pressed',active?'true':'false');});
    updateExperienceCard();updateControlCenterCopy();renderPreviewBadge();
  }

  async function persistType(){
    if(!studioActive()||persistBusy)return;
    let id=null;try{id=typeof currentId!=='undefined'?currentId:null;}catch(_){ }
    if(!id||!window.supabaseClient?.rpc)return;
    persistBusy=true;
    try{const {error}=await window.supabaseClient.rpc('set_studio_business_type',{p_card_id:id,p_business_type:selectedType});if(error)throw error;}
    catch(error){console.warn('Studio business type save failed:',error);try{window.toast?.('Studio type will retry with your next save.');}catch(_){ }}
    finally{persistBusy=false;}
  }

  function chooseType(type,{persist=true}={}){
    selectedType=TYPES[type]?type:'barber';syncButtons();if(persist)void persistType();try{if(typeof render==='function')render();}catch(_){ }
  }

  function mountPicker(){
    updateExperienceCard();updateControlCenterCopy();
    const look=q('#barber-control-center [data-barber-v5-panel="look"]');if(!look)return false;
    let picker=q('[data-studio-business-picker]',look);
    if(!picker){
      picker=document.createElement('section');picker.className='studio-business-picker';picker.dataset.studioBusinessPicker='true';
      ...
      const bridge=q('[data-barber-template-bridge]',look);if(bridge)bridge.insertAdjacentElement('beforebegin',picker);else look.prepend(picker);
      picker.addEventListener('click',event=>{const button=event.target.closest('[data-studio-business-type]');if(button)chooseType(button.dataset.studioBusinessType);});
    }
  ...
  async function loadSavedType(){
    let id=null;try{id=typeof currentId!=='undefined'?currentId:null;}catch(_){ }
    if(!id||!window.supabaseClient||loadedCardId===String(id))return;
    loadedCardId=String(id);
    try{const {data,error}=await window.supabaseClient.from('digital_cards').select('studio_business_type').eq('id',id).maybeSingle();if(!error&&data?.studio_business_type)chooseType(String(data.studio_business_type),{persist:false});}
    catch(error){console.warn('Studio business type load skipped:',error);}
  }

  function refresh(){injectStyles();updateExperienceCard();updateControlCenterCopy();mountPicker();syncButtons();void loadSavedType();}

  const priorPerformSave=typeof performSave==='function'?performSave:null;
  if(priorPerformSave){performSave=async function(...args){const result=await priorPerformSave.apply(this,args);if(studioActive())await persistType();return result;};}
