...
(function(){
  ...
  function statusLabel(status){if(status==='approved'&&payload?.approval?.published_at)return 'Approved · Live';return ({sent:'Awaiting your response',approved:'Approved',changes_requested:'Changes requested'})[status]||'Ready for review';}
  ...
  function renderDecision(){
    ...
    if(status==='sent'&&payload.approval?.auto_publish){
      const copy=$('#agency-review-decision>p');if(copy)copy.textContent='Approve this card to let the agency publish it automatically. If anything needs changing, send your notes instead.';
    }
    if(status==='approved'){
      approve.hidden=true;request.hidden=true;final.hidden=false;
      const published=Boolean(payload.approval?.published_at);
      const publishError=clean(payload.approval?.publish_error);
      final.className='agency-review-final';
      if(published){
        final.innerHTML=`<strong>Approved and live.</strong><br>The card has been published${payload.approval?.publish_mode==='auto'?' automatically':''}.`;
      }else if(publishError){
        final.innerHTML='<strong>Approved.</strong><br>Your approval is complete. The agency has been notified and will finish publishing the card.';
      }else{
        final.innerHTML='<strong>Approved.</strong><br>Your agency has been notified and the card is ready to publish.';
      }
    }else if(status==='changes_requested'){
      approve.hidden=true;request.hidden=true;final.hidden=false;final.className='agency-review-final changes';final.innerHTML=`<strong>Changes requested.</strong><br>${esc(payload.approval?.client_feedback||'Your notes were sent to the agency.')}`;
    }
  }
  ...
  async function submitAction(action,feedback=''){
    ...
    try{
      const result=await invoke({token,action,feedback});
      payload.approval={...payload.approval,...result.approval};
      if(result.publishError)payload.approval.publish_error=result.publishError;
      if(result.autoPublished&&payload.preview?.card)payload.preview.card.status='published';
      payload.publicUrl=result.publicUrl||payload.publicUrl||'';
      renderStatus();renderDecision();
      toast(action==='approved'?(result.autoPublished?'Approved — your card is live.':'Card approved. Thank you.'):'Your requested changes were sent.');
    }
