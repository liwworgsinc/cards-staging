...
(function(){
    ...
    if(!q('.realtor-editor-panel')){
      ...
      panel.innerHTML=`
        ...
        <div class="realtor-tool-panel" data-realtor-panel="design">
          <div class="form-section"><div class="section-mini-heading"><div><h3>LIW Card Design</h3><p>Use the same LIW design families: Classic, Flow, Showtime or Studio. Listings stay exactly the same when you switch.</p></div></div><div class="realtor-preset-grid" id="realtor-presets"></div></div>
        </div>`;
  ...
  function renderPresets(){
    ...
    const current=normalizePreset(settings.style_preset);settings.style_preset=current;
    host.innerHTML=Object.entries(PRESETS).map(([key,p])=>`<button type="button" class="realtor-preset ${current===key?'active':''}" data-realtor-preset="${key}"><span class="realtor-preset-swatch" style="background:${p.surface};border:1px solid rgba(0,0,0,.06)"><i style="background:${p.dark}"></i><i style="background:${p.accent};margin-left:4px"></i></span><strong>${p.label}</strong><small>${p.description}</small></button>`).join('');
  ...
  function renderPreview(){
    ...
    const styleKey=normalizePreset(settings.style_preset),p=preset(),visible=listings.filter(l=>l.is_visible!==false),featured=visible.find(l=>l.is_featured)||visible.find(l=>l.status!=='sold')||visible[0];
