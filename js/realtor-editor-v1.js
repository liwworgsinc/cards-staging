...
(function(){
  ...
  const GALLERY_SLOTS=[
    {label:'Kitchen',icon:'cooking-pot'},
    {label:'Bathroom',icon:'bath'},
    {label:'Primary Bedroom',icon:'bed-double'},
    {label:'Living Room',icon:'sofa'},
    {label:'Exterior / Backyard',icon:'trees'},
    {label:'Other Highlight',icon:'image-plus'}
  ];
  const PRESETS={
  ...
  function ensureEditorUi(){
    ...
    if(!q('.realtor-control-center')){
      ...
      panel.innerHTML=`
        ...
        <div class="realtor-tool-tabs"><button type="button" class="active" data-realtor-tab="agent">Agent</button><button type="button" data-realtor-tab="listings">Listings</button><button type="button" data-realtor-tab="appointments">Appointments &amp; Showings</button><button type="button" data-realtor-tab="design">Card Design</button></div>
        <div class="realtor-tool-panel active" data-realtor-panel="agent">
          <div class="form-section"><div class="section-mini-heading"><div><h3>Agent details</h3><p>These sit alongside the normal name, photo, phone and email already on your LIW Card.</p></div><span class="realtor-status" id="realtor-save-status">Autosaves</span></div>
            ...
            <div class="realtor-video-cover"><strong>Video cover</strong><p class="muted">Upload a short property or branding clip. LIW optimizes larger files. Your cover image stays as the fallback poster.</p><div class="realtor-video-cover-actions"><label class="btn btn-primary btn-sm" for="realtor-video-cover-file"><i data-lucide="video" size="15"></i> Upload video</label><input id="realtor-video-cover-file" type="file" accept="video/mp4,video/webm,video/quicktime" hidden><button class="btn btn-ghost btn-sm" id="realtor-remove-video-cover" type="button">Remove video</button></div><div class="realtor-video-progress" id="realtor-video-progress">Optimizing video…</div><video class="realtor-video-cover-preview" id="realtor-video-cover-preview" muted loop playsinline autoplay preload="metadata"></video></div><div class="form-row"><div class="form-group"><label>Brokerage logo URL <span class="muted">optional</span></label><input class="input" data-realtor-setting="brokerage_logo_url" type="url" placeholder="https://..."></div><div class="form-group"><label>Or upload logo</label><input class="input" id="realtor-logo-file" type="file" accept="image/jpeg,image/png,image/webp"></div></div>
          </div>
        </div>
        ...
        <div class="realtor-tool-panel" data-realtor-panel="appointments">
  ...
  function renderListings(){
    ...
    host.innerHTML=listings.map((l,index)=>`<article class="realtor-listing-card" data-realtor-index="${index}"><div class="realtor-listing-head"><div><strong>${esc(l.address||`Listing ${index+1}`)}</strong> ${l.is_featured?'<span class="realtor-badge gold">Featured</span>':''}</div><span class="realtor-badge status-${esc(l.status||'for_sale')}">${statusLabel(l.status)}</span></div><div class="realtor-listing-body">
      ...
      <div class="form-row"><select class="input realtor-quick-select" data-k="status"><option value="for_sale">For Sale</option><option value="new_listing">New Listing</option><option value="coming_soon">Coming Soon</option><option value="open_house">Open House</option><option value="under_contract">Under Contract</option><option value="pending">Pending</option><option value="sold">Sold</option></select><select class="input realtor-quick-select" data-k="property_type">${propertyTypeOptions(l.property_type)}</select></div>
    ...
    qa('[data-realtor-index]',host).forEach(row=>{
      ...
      qa('[data-gallery-photo]',row).forEach(input=>input.addEventListener('change',event=>uploadGalleryPhoto(event,index,Number(input.dataset.galleryPhoto))));
      qa('[data-gallery-remove]',row).forEach(button=>button.addEventListener('click',()=>removeGalleryPhoto(index,Number(button.dataset.galleryRemove))));
    });
    if(window.lucide)try{lucide.createIcons();}catch(_){ }
  }
  ...
  async function uploadListingPhoto(event,index){const file=event.target.files?.[0];if(!file)return;try{listings[index].main_image_url=await uploadImage(file,'listings/main');queueSave(true);renderListings();renderPreview();toast('Main property photo uploaded');}catch(error){toast(error.message||'Unable to upload photo');}}
  async function uploadGalleryPhoto(event,index,slotIndex){const file=event.target.files?.[0];if(!file||!GALLERY_SLOTS[slotIndex])return;try{const urls=gallerySlots(listings[index]);urls[slotIndex]=await uploadImage(file,`listings/gallery/${slotIndex+1}`);listings[index].gallery_urls=urls;queueSave(true);renderListings();renderPreview();toast(`${GALLERY_SLOTS[slotIndex].label} photo uploaded`);}catch(error){toast(error.message||'Unable to upload gallery photo');}}
  function removeGalleryPhoto(index,slotIndex){if(!listings[index]||!GALLERY_SLOTS[slotIndex])return;const urls=gallerySlots(listings[index]);urls[slotIndex]='';listings[index].gallery_urls=urls;queueSave(true);renderListings();renderPreview();toast(`${GALLERY_SLOTS[slotIndex].label} photo removed`);}
  ...
  function featureCard(l){if(!l)return '<div class="realtor-empty">Add a listing in Realtor Tools to feature a property.</div>';const price=l.status==='sold'&&l.sold_price_cents?l.sold_price_cents:l.price_cents,bedText=bedsLabel(l.beds);return `<article class="realtor-feature"><div class="realtor-feature-photo" style="${l.main_image_url?`background-image:url('${esc(l.main_image_url)}')`:''}"><span class="realtor-feature-badge status-${esc(l.status||'for_sale')}">${statusLabel(l.status)}</span></div><div class="realtor-feature-copy"><strong>${esc(address(l)||'Property address')}</strong><b>${money(price)||'Price on request'}</b><div class="realtor-feature-meta">${bedText?`<span>${esc(bedText)}</span>`:''}${l.baths!=null?`<span>${esc(l.baths)} Baths</span>`:''}${l.square_feet?`<span>${esc(l.square_feet)} Sq Ft</span>`:''}</div><div class="realtor-feature-buttons"><span>View Property</span><span>${settings.showing_enabled&&settings.showing_service_id?'Schedule Showing':'Request Showing'}</span></div></div></article>`;}
