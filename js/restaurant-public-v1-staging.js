...
(function(){
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      .restaurant-public-hero{min-height:315px;position:relative;background:linear-gradient(145deg,var(--rest-dark),#2b130a);background-size:cover;background-position:center;color:#fff}.restaurant-public-hero:after{content:'';position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,.05),rgba(0,0,0,.76))}
  ...
  function infoDrawer(cardData){
    ...
    const buttons=[];
    if(String(cardData.biography||'').trim())buttons.push('<button class="restaurant-info-item" data-rest-info="about"><i data-lucide="info" size="16"></i><span>About</span></button>');
    ...
    if(address)buttons.push('<button class="restaurant-info-item" data-rest-info="location"><i data-lucide="map-pin" size="16"></i><span>Location</span></button>');
    ...
    if(utility.socials.length)buttons.push('<button class="restaurant-info-item" data-rest-info="social"><i data-lucide="users-round" size="16"></i><span>Social</span></button>');
    if(!buttons.length)return'';
    return '<div class="restaurant-info-control"><button class="restaurant-info-toggle" type="button" data-rest-info-toggle><span><i data-lucide="store" size="15"></i> Restaurant Info</span><i data-lucide="chevron-down" size="15"></i></button><div class="restaurant-info-drawer" data-rest-info-drawer hidden>'+buttons.join('')+'</div></div>';
  }
  ...
  function showInfo(type,cardData){
    ...
    if(type==='social'){
      const first=utility.socials.find(x=>x.url);if(first?.url)window.open(normalize(first.url),'_blank','noopener');
      return;
    }
    if(type==='hours'){
      const hours=section('hours');
      const text=hours?.content?.days?hours.content.days.map(d=>(d.day||d.label||'')+' '+(d.open||'')+(d.close?'–'+d.close:'')).join('\n'):'Hours are available from the restaurant.';
      alert(text);
    }
  }
  ...
  function bind(cardData){
    ...
    qa('[data-rest-info]',shellEl).forEach(btn=>btn.addEventListener('click',()=>showInfo(btn.dataset.restInfo,cardData)));
  }
