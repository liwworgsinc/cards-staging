...
(function(){
  ...
  let settings={cuisine:'',service_style:'',tagline:'',price_note:'',order_url:'',reservation_url:'',delivery_note:'',featured_item_name:'',video_cover_url:''};
  ...
  async function saveSettings(){
    ...
    try{
      const clean={cuisine:settings.cuisine||'',service_style:settings.service_style||'',tagline:settings.tagline||'',price_note:settings.price_note||'',order_url:settings.order_url||'',reservation_url:settings.reservation_url||'',delivery_note:settings.delivery_note||'',featured_item_name:settings.featured_item_name||'',video_cover_url:settings.video_cover_url||''};
  ...
  async function uploadRestaurantVideo(event){
    ...
    try{
      ...
      const {error}=await supabaseClient.storage.from('profile-images').upload(path,file,{cacheControl:'3600',upsert:false,contentType:file.type||'video/webm'});
      if(error)throw error;
