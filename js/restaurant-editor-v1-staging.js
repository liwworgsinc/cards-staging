...
(function(){
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      .restaurant-phone-identity h3{font-family:Georgia,'Times New Roman',serif;font-size:1.75rem!important;letter-spacing:-.03em;text-shadow:0 2px 10px rgba(0,0,0,.3)}
  ...
  function renderPreview(){
    ...
    shell.innerHTML=`<div class="restaurant-phone-hero" style="${heroStyle}">${videoHero}<div class="restaurant-phone-top"><div class="restaurant-phone-brand-lockup"><div class="restaurant-phone-brand-mark">${brandImage?`<img src="${esc(brandImage)}" alt="">`:'♨'}</div><div class="restaurant-phone-brand-copy"><div class="restaurant-phone-brand">${esc(restName)}</div><div class="restaurant-phone-brand-sub">Dining experience</div></div></div><div class="restaurant-phone-top-actions"><span>↗</span><span>▦</span><span>▱</span></div></div><div class="restaurant-phone-identity"><span class="restaurant-phone-kicker">FOOD & DINING</span><h3>${esc(restName)}</h3><p>${esc([settings.cuisine,settings.service_style,settings.price_note].filter(Boolean).join(' · ')||owner||'Food & Dining')}</p>${statuses}</div></div><div class="restaurant-phone-body"><div class="restaurant-phone-actions"><span><i>♨</i>Menu</span><span><i>▢</i>Order</span><span><i>◷</i>Reserve</span><span><i>⌂</i>Info</span></div><div class="restaurant-phone-tagline">${esc(settings.tagline||value('headline')||'More than a meal. A reason to come back.')}</div><section class="restaurant-phone-signature"><div class="restaurant-phone-signature-top"><div><div class="restaurant-phone-signature-eyebrow">A TASTE OF ${esc(restName.toUpperCase())}</div><h3>Signature Dish</h3></div>${items.length?'<a href="javascript:void(0)">See Menu →</a>':''}</div>${featureHtml}</section>${chefMini}${mini?`<div class="restaurant-phone-section-head"><strong>Menu Highlights</strong><span>Swipe · ${items.length}</span></div><div class="restaurant-menu-mini">${mini}</div>`:''}<div class="restaurant-phone-cta"><strong>Ready to dine?</strong><p>${esc(settings.delivery_note||'Order ahead or reserve your table.')}</p><div class="restaurant-phone-cta-row"><span>Order Online</span><span>Reserve Table</span></div></div></div>`;
  }
