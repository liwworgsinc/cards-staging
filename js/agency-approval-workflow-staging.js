...
(function(){
  ...
  function reviewState(card,approval){if(!approval)return 'draft';if(approval.status==='approved')return card?.status==='published'?'live':'ready';return String(approval.status||'draft');}
  ...
  function readyCards(){return Array.from(state.cards.values()).filter(card=>{const approval=state.approvals.get(String(card.id));return approval?.status==='approved'&&card.status!=='published';}).sort((a,b)=>cardLabel(a).localeCompare(cardLabel(b)));}
  ...
  function ensureApprovalSummary(){
    ...
    if(!panel){panel=document.createElement('div');panel.dataset.agencyApprovalSummary='true';panel.className='agency-approval-summary';const anchor=section.querySelector('.agency-cards-summary')||section.querySelector('.agency-section-head');anchor?.insertAdjacentElement('afterend',panel);}
    const rows=Array.from(state.approvals.values());
    const pending=rows.filter(r=>r.status==='sent').length,ready=readyCards().length,changes=rows.filter(r=>r.status==='changes_requested').length;
  ...
  function cardIdFromTile(tile){const edit=tile.querySelector('a[href*="editor.html?id="]');if(!edit)return '';try{return new URL(edit.getAttribute('href'),location.href).searchParams.get('id')||'';}catch(_){return '';}}
  ...
  function enhanceCards(){
    ...
    grid.querySelectorAll('.agency-client-card').forEach(tile=>{
      ...
      if(row.dataset.approvalRender!==rowSignature){row.dataset.approvalRender=rowSignature;const detailMarkup=detail?`<small title="${esc(detail)}">${esc(detail)}</small>`:'';row.className=`agency-card-approval-state status-${status}`;row.innerHTML=`<span><i data-lucide="${iconFor(status)}" size="14"></i>${labelFor(status)}</span>${detailMarkup}`;iconsChanged=true;}
      ...
      if(status==='ready'){
        if(!publishButton){publishButton=document.createElement('button');publishButton.type='button';publishButton.className='btn btn-publish-approved btn-sm';publishButton.dataset.publishApprovedCard=cardId;publishButton.addEventListener('click',()=>publishApprovedCard(cardId,publishButton));actions.prepend(publishButton);iconsChanged=true;}
        const publishing=state.publishing.has(cardId);const publishSignature=publishing?'publishing':'ready';
        if(publishButton.dataset.publishRender!==publishSignature){publishButton.dataset.publishRender=publishSignature;publishButton.disabled=publishing;publishButton.innerHTML=publishing?'<span class="spinner-mini"></span> Publishing…':'<i data-lucide="rocket" size="14"></i>Publish card';iconsChanged=true;}
      ...
      const disabled=state.used>=state.limit,buttonSignature=`${status}|${disabled?'1':'0'}|${state.limit}`;
      if(button.dataset.approvalRender!==buttonSignature){button.dataset.approvalRender=buttonSignature;button.innerHTML=`<i data-lucide="mail-check" size="14"></i>${buttonFor(status)}`;button.disabled=disabled;button.title=disabled?`Monthly approval email limit reached (${state.limit}).`:buttonFor(status);iconsChanged=true;}
    });
    if(iconsChanged&&window.lucide)try{lucide.createIcons();}catch(_){}
  }

  function cardLabel(card){const client=card.agency_client_id?state.clients.get(String(card.agency_client_id)):null;const name=clean(card.full_name||client?.name)||'Untitled card';const company=clean(card.company_name||client?.company_name);return company?`${name} — ${company}`:name;}

  function setDialogCard(cardId){
    const card=state.cards.get(String(cardId));if(!card)return;
    const client=card.agency_client_id?state.clients.get(String(card.agency_client_id)):null,approval=state.approvals.get(String(cardId));
    $('#agency-approval-card-id').value=String(cardId);$('#agency-approval-card-name').textContent=card.full_name||client?.name||'Client card';$('#agency-approval-card-company').textContent=card.company_name||client?.company_name||'';$('#agency-approval-current-status').textContent=labelFor(reviewState(card,approval));$('#agency-approval-email').value=approval?.recipient_email||client?.email||card.email||'';
  }
  ...
  function populateCardSelect(selectedId=''){
    ...
    const chosen=state.cards.has(String(selectedId))?String(selectedId):String(cards[0]?.id||'');if(chosen){select.value=chosen;setDialogCard(chosen);}return chosen;
  }
  ...
  function ensureDialog(){
    ...
    document.body.appendChild(dialog);dialog.querySelectorAll('[data-close-approval-dialog]').forEach(btn=>btn.addEventListener('click',()=>dialog.close()));dialog.querySelector('form')?.addEventListener('submit',sendApproval);$('#agency-approval-card-select')?.addEventListener('change',event=>setDialogCard(event.target.value));if(window.lucide)try{lucide.createIcons();}catch(_){}return dialog;
  }

  function openSendDialog(cardId=''){
    if(!state.cards.size){notify('Create an Agency card first.');return;}
    const dialog=ensureDialog();populateCardSelect(cardId);$('#agency-approval-message').value='';$('#agency-approval-auto-publish').checked=false;$('#agency-approval-dialog-quota-text').textContent=`${state.used} / ${state.limit}`;$('#agency-approval-dialog-quota-copy').textContent=`${Math.max(0,state.limit-state.used)} approval emails remaining this month.`;$('#agency-approval-send-button').disabled=state.used>=state.limit;if(!dialog.open)dialog.showModal();setTimeout(()=>$('#agency-approval-email')?.focus(),80);
  }

  function ensureReadyDialog(){
    let dialog=$('#agency-ready-publish-dialog');if(dialog)return dialog;
    dialog=document.createElement('dialog');dialog.id='agency-ready-publish-dialog';dialog.className='agency-dialog agency-ready-publish-dialog';dialog.innerHTML=`<div class="agency-dialog-body"><div class="agency-dialog-head"><div><span class="agency-approval-kicker">APPROVED</span><h2>Ready to publish</h2><p>Client-approved drafts can go live right here. No editor detour required.</p></div><button class="icon-btn" type="button" data-close-ready-dialog aria-label="Close"><i data-lucide="x"></i></button></div><div class="agency-ready-publish-list" id="agency-ready-publish-list"></div><div class="agency-dialog-actions"><button class="btn btn-light" type="button" data-close-ready-dialog>Close</button></div></div>`;document.body.appendChild(dialog);dialog.querySelectorAll('[data-close-ready-dialog]').forEach(btn=>btn.addEventListener('click',()=>dialog.close()));return dialog;
  }
  ...
  function renderReadyDialog(){
    ...
    list.querySelectorAll('[data-ready-publish-card]').forEach(button=>button.addEventListener('click',()=>publishApprovedCard(button.dataset.readyPublishCard,button)));if(window.lucide)try{lucide.createIcons();}catch(_){}return dialog;
  }
  function openReadyDialog(){const dialog=renderReadyDialog();if(!dialog.open)dialog.showModal();}

  async function edgeErrorMessage(error,fallback){try{if(error?.context?.json){const body=await error.context.json();if(body?.error)return body.error;}}catch(_){ }return error?.message||fallback;}

  async function sendApproval(event){
    event.preventDefault();if(state.busy)return;
    const cardId=clean($('#agency-approval-card-id')?.value),recipientEmail=clean($('#agency-approval-email')?.value),message=clean($('#agency-approval-message')?.value),autoPublish=Boolean($('#agency-approval-auto-publish')?.checked);if(!cardId||!recipientEmail)return;
    ...
    try{const {data,error}=await supabaseClient.functions.invoke('send-agency-approval',{body:{cardId,recipientEmail,message,autoPublish,environment:'staging',previewPlan:state.planKey}});if(error)throw new Error(await edgeErrorMessage(error,'Approval email could not be sent.'));if(data?.error)throw new Error(data.error);$('#agency-approval-send-dialog')?.close();notify(autoPublish?'Approval sent · auto-publish is on.':'Approval email sent.');await refresh();}
  ...
  async function publishApprovedCard(cardId,button=null){
    ...
    state.publishing.add(cardId);const original=button?.innerHTML||'';if(button){button.disabled=true;button.innerHTML='<span class="spinner-mini"></span> Publishing…';}
    try{const {data,error}=await supabaseClient.functions.invoke('publish-agency-approved-card',{body:{cardId,environment:'staging',previewPlan:state.planKey}});if(error)throw new Error(await edgeErrorMessage(error,'Approved card could not be published.'));if(data?.error)throw new Error(data.error);notify(data?.alreadyPublished?'Card is already live.':'Card published — it’s live.');await refresh();const readyDialog=$('#agency-ready-publish-dialog');if(readyDialog?.open){renderReadyDialog();if(!readyCards().length)setTimeout(()=>readyDialog.close(),650);}}
  ...
  function observe(){const section=$('#cards');if(!section||state.observer)return;let frame=0;state.observer=new MutationObserver(()=>{cancelAnimationFrame(frame);frame=requestAnimationFrame(enhanceCards);});state.observer.observe(section,{childList:true,subtree:true});}
  async function boot(){try{await resolveContext();await loadData();ensureDialog();ensureReadyDialog();ensureApprovalSummary();enhanceCards();observe();setInterval(()=>{if(!document.hidden)refresh();},20000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)refresh();});}catch(error){console.warn('Agency approval workflow:',error);}}
