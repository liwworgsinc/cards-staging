...
(function(){
  'use strict';
  const RUNTIME_VERSION='20260918-realtor-info-1';
  ...
  const utilityStripMarkup=cardData=>{
    ...
    const items=[];
    if(website)items.push(`<a class="realtor-utility-item" href="${esc(website)}" target="_blank" rel="noopener"><i data-lucide="globe-2" size="16"></i><span>Website</span></a>`);
    if(hours?.content?.days?.length)items.push('<button class="realtor-utility-item" type="button" data-realtor-info="hours"><i data-lucide="clock-3" size="16"></i><span>Hours</span></button>');
    if(addressText)items.push('<button class="realtor-utility-item" type="button" data-realtor-info="location"><i data-lucide="map-pin" size="16"></i><span>Location</span></button>');
    if(utilityData.socials.length)items.push('<button class="realtor-utility-item" type="button" data-realtor-info="social"><i data-lucide="share-2" size="16"></i><span>Social</span></button>');
    return items.length?`<div class="realtor-utility-strip" aria-label="Agent information">${items.join('')}</div>`:'';
  };
  ...
  function bindShell(cardData){
    ...
    qa('[data-realtor-info]',shell).forEach(btn=>btn.addEventListener('click',()=>openInfo(btn.dataset.realtorInfo,cardData)));
  }
