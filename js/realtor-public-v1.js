...
const RUNTIME_VERSION='20260926-realtor-listing-hours-1';
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      .realtor-property-actions .realtor-showing-action svg{flex:0 0 auto;stroke:#fff!important;color:#fff!important}
      .realtor-property-actions [data-realtor-lead="info"]{background:color-mix(in srgb,var(--raccent,#c6a15b) 20%,#fff)!important;color:#111827!important;border:1px solid #d8dce5!important}
  ...
  const showingConfigured=l=>Boolean(card()?.realtor_settings?.showing_enabled&&card()?.realtor_settings?.showing_service_id&&
    card()?.realtor_settings?.showing_schedules?.[l?.id]?.enabled);
  const canShow=l=>!['sold','under_contract','pending'].includes(String(l?.status||'').toLowerCase());
  const showingAction=l=>!canShow(l)?'':showingConfigured(l)
    ?'<button class="realtor-showing-action" type="button" data-realtor-booking="showing" data-listing-id="'+esc(l.id)+'"><i data-lucide="calendar-days" size="16"></i> Schedule a Showing</button>'
    :'<button class="realtor-showing-action" type="button" data-realtor-lead="showing" data-listing-id="'+esc(l.id)+'"><i data-lucide="calendar-days" size="16"></i> Request a Showing</button>';
  ...
  function openNativeBooking(context=null){
    if(context&&!showingConfigured(listings.find(item=>String(item.id)===String(context.id))))return;
  ...
  async function mount(){
    ...
    document.body?.classList.add('realtor-public-page');
    q('.public-shell')?.classList.add('realtor-public-shell-host');
    let shell=q('#realtor-public-shell');if(shell)shell.remove();article.insertAdjacentHTML('beforeend',shellHtml(cardData));bindShell(cardData);if(listings.some(l=>showingConfigured(l))||cardData.realtor_settings?.consultation_enabled)q('#realtor-public-shell')?.setAttribute('data-liw-native-showing','true');bindStickyHero();
    if(window.lucide)try{lucide.createIcons();}catch(_){ }
