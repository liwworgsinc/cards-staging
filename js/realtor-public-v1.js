...
const RUNTIME_VERSION='20260924-realtor-showing-native-1';
  ...
  function injectStyles(){
    ...
    style.textContent=`
      ...
      .realtor-main-showing{width:100%;min-height:51px;display:flex;align-items:center;justify-content:center;gap:9px;border:0;border-radius:var(--rradius,13px);background:var(--rbutton,var(--rdark));color:var(--rbuttontext,#fff);font:inherit;font-size:.84rem;font-weight:900;cursor:pointer}
      .realtor-booking-picker{display:grid;gap:9px}.realtor-booking-picker button{display:flex;align-items:center;gap:10px;min-height:50px;text-align:left;border:1px solid #e2e8f0;border-radius:13px;background:#f8fafc;color:#111827;padding:12px;font:inherit;font-weight:800;cursor:pointer}
  ...
  const utilityStripMarkup=cardData=>{
    ...
    if(cardData.realtor_settings?.consultation_enabled)items.push('<button class="realtor-office-item" type="button" data-realtor-booking="general"><i data-lucide="calendar-days" size="16"></i><span>Book Consultation</span></button>');
  ...
  const showingConfigured=()=>Boolean(card()?.realtor_settings?.showing_enabled&&card()?.realtor_settings?.showing_service_id);
  const canShow=l=>!['sold','under_contract','pending'].includes(String(l?.status||'').toLowerCase());
  const showingAction=l=>!canShow(l)?'<button type="button" data-realtor-lead="info" data-listing-id="'+esc(l.id)+'">Ask About It</button>':showingConfigured()
    ?'<button type="button" data-realtor-booking="showing" data-listing-id="'+esc(l.id)+'">Schedule a Showing</button>'
    :'<button type="button" data-realtor-lead="showing" data-listing-id="'+esc(l.id)+'">Request Showing</button>';
  ...
  function ensureDialogs(){
    ...
    if(!q('#realtor-booking-dialog')){
      const dialog=document.createElement('dialog');dialog.id='realtor-booking-dialog';dialog.className='realtor-modal';dialog.innerHTML='<div class="realtor-modal-body"><div class="realtor-modal-head"><div><h2 id="realtor-booking-heading">Schedule a Showing</h2><p class="muted" id="realtor-booking-subtitle">Select a date and time.</p></div><button type="button" class="realtor-modal-close" data-close-realtor-booking aria-label="Close booking"><i data-lucide="x"></i></button></div><div id="realtor-booking-dialog-body"></div></div>';document.body.appendChild(dialog);
      dialog.querySelector('[data-close-realtor-booking]')?.addEventListener('click',()=>{pendingNativeBooking=null;dialog.close();});
    }
  ...
  function openProperty(id){
    ...
    body.innerHTML=`<div class="realtor-modal-head"><div><h2>${esc(address(l)||'Property')}</h2><div class="realtor-meta"><span>${statusLabel(l.status)}</span>${money(price)?`<strong>${money(price)}</strong>`:''}</div></div><button class="realtor-modal-close" type="button" data-close-realtor><i data-lucide="x"></i></button></div>${gallery.length?`<div class="realtor-gallery">${gallery.map(url=>`<img src="${esc(url)}" alt="Property photo" loading="lazy">`).join('')}</div>`:''}<div class="realtor-detail-copy"><div class="realtor-meta">${bedText?`<span>${esc(bedText)}</span>`:''}${l.baths!=null?`<span>${esc(l.baths)} Baths</span>`:''}${l.square_feet?`<span>${esc(l.square_feet)} Sq Ft</span>`:''}${l.property_type?`<span>${esc(l.property_type)}</span>`:''}${l.mls_number?`<span>MLS ${esc(l.mls_number)}</span>`:''}</div>${l.description?`<p>${esc(l.description)}</p>`:''}<div class="realtor-property-actions">${showingAction(l)}<button type="button" data-realtor-lead="info" data-listing-id="${esc(l.id)}">Ask About It</button>${l.virtual_tour_url?`<a href="${esc(normalize(l.virtual_tour_url))}" target="_blank" rel="noopener">Virtual Tour</a>`:l.external_url?`<a href="${esc(normalize(l.external_url))}" target="_blank" rel="noopener">Listing Details</a>`:`''`}</div></div>`;
  ...
  function bookingDialog(title,subtitle){
    ...
    return q('#realtor-booking-dialog-body',dialog);
  }
  function openNativeBooking(context=null){
    if(context&&!showingConfigured())return;
    pendingNativeBooking=context?{...context}:{general:true};
    const body=bookingDialog(context?'Schedule a Showing':'Book a Consultation',context?context.address:'Choose an appointment service.');
    const api=window.LIWNativeBookingV1;
    if(!api){body.innerHTML='<p class="muted">Loading available appointments…</p>';return;}
    const result=context?api.openForListing(context):api.openGeneral();
    if(result.ok){pendingNativeBooking=null;return;}
    if(result.reason==='loading'){body.innerHTML='<p class="muted">Loading available appointments…</p>';return;}
  ...
  function openShowingPicker(){
    const available=listings.filter(canShow);
  ...
  function bindShell(cardData){
    ...
    q('[data-realtor-share]',shell)?.addEventListener('click',()=>{try{if(typeof window.LIWCardShare?.open==='function')window.LIWCardShare.open();else if(typeof shareCard==='function')shareCard();else navigator.share?.({title:cardData.full_name||'Real Estate',url:location.href});}catch(_){ }});
  ...
  async function mount(){
    ...
    document.body?.classList.add('realtor-public-page');
    q('.public-shell')?.classList.add('realtor-public-shell-host');
    let shell=q('#realtor-public-shell');if(shell)shell.remove();article.insertAdjacentHTML('beforeend',shellHtml(cardData));bindShell(cardData);if(showingConfigured()||cardData.realtor_settings?.consultation_enabled)q('#realtor-public-shell')?.setAttribute('data-liw-native-showing','true');bindStickyHero();
    if(window.lucide)try{lucide.createIcons();}catch(_){ }
