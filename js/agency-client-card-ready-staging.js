...
(function(){
    ...
    try{
      ...
      if(base?.isAdmin&&typeof isLiwStagingPlanQaHost==='function'&&isLiwStagingPlanQaHost()){
        ...
        if(['agency','white_label'].includes(preview)){
          ...
          flowAllowed=Boolean(plan?.entitlements?.flow_experience);
          return;
        }
      }
  ...
  function renderTemplateOptions(){
    const select=document.getElementById(TEMPLATE_SELECT_ID);
    if(!select)return;
  ...
  function formMarkup(){
    return `
      <div class="agency-dialog-head agency-card-ready-head">
        <div><span class="agency-card-ready-kicker">CARD-READY CLIENT</span><h2>Add client</h2><p>Add only what helps build the card. Finish photos, socials, and advanced tools in the editor.</p></div>
        <button class="icon-btn" type="button" data-card-ready-close aria-label="Close"><i data-lucide="x"></i></button>
      </div>

      <div class="agency-card-ready-section">
        <div class="agency-card-ready-section-head"><span class="agency-card-ready-step">1</span><div><strong>Who is this card for?</strong><small>Name is the only required field.</small></div></div>
        <div class="agency-form-grid agency-card-ready-grid">
          <div class="agency-field full"><label for="agency-client-name">Full name *</label><input id="agency-client-name" required maxlength="160" autocomplete="name" placeholder="Jane Smith"></div>
          <div class="agency-field"><label for="agency-client-job-title">Job title or specialty</label><input id="agency-client-job-title" maxlength="180" placeholder="Licensed Real Estate Agent"></div>
          <div class="agency-field"><label for="agency-client-company">Business or company</label><input id="agency-client-company" maxlength="180" autocomplete="organization" placeholder="Smith Realty"></div>
        </div>
      </div>

      <div class="agency-card-ready-section">
        <div class="agency-card-ready-section-head"><span class="agency-card-ready-step">2</span><div><strong>How should customers reach them?</strong><small>Add the contact methods you already have.</small></div></div>
        <div class="agency-form-grid agency-card-ready-grid">
          <div class="agency-field"><label for="agency-client-email">Email</label><input id="agency-client-email" type="email" maxlength="220" autocomplete="email" placeholder="jane@company.com"></div>
          <div class="agency-field"><label for="agency-client-phone">Phone</label><input id="agency-client-phone" inputmode="tel" maxlength="80" autocomplete="tel" placeholder="(555) 555-5555"></div>
          <div class="agency-field full"><label for="agency-client-website">Website</label><input id="agency-client-website" inputmode="url" maxlength="500" placeholder="https://"></div>
        </div>
      </div>

      <div class="agency-card-ready-section agency-card-ready-build">
        <label class="agency-card-ready-toggle" for="${CREATE_CARD_ID}">
          <span class="agency-card-ready-toggle-icon"><i data-lucide="badge-plus" size="18"></i></span>
          <span><strong>Create a draft card now</strong><small>Recommended — the contact info above will populate the card automatically.</small></span>
          <input id="${CREATE_CARD_ID}" type="checkbox" checked><span class="agency-switch" aria-hidden="true"></span>
        </label>
        <div class="agency-card-ready-card-options" data-card-ready-card-options>
          <div class="agency-form-grid agency-card-ready-grid">
            <div class="agency-field full"><label for="${TEMPLATE_SELECT_ID}">Start from a design</label><select id="${TEMPLATE_SELECT_ID}"><option value="">Start with default LIW card</option></select><small class="agency-field-help">Agency Templates copy the design, not another client's personal details.</small></div>
            <div class="agency-field full"><label>Card experience</label><div class="agency-experience-picker" id="${EXPERIENCE_ID}"><button type="button" class="active" data-agency-experience="classic"><strong>Standard</strong><span>Simple vertical card</span></button><button type="button" data-agency-experience="flow"><strong>Flow</strong><span>App-like swipe experience</span></button></div><input type="hidden" id="agency-client-experience-value" value="classic"></div>
          </div>
        </div>
      </div>
  ...
  function installForm(){
    const dialog=document.getElementById(DIALOG_ID),form=document.getElementById(FORM_ID);
    if(!dialog||!form||form.dataset.cardReadyInstalled==='true')return false;
    ...
    document.getElementById(CREATE_CARD_ID)?.addEventListener('change',syncCreateCardUi);
    form.addEventListener('submit',submitCardReadyClient,true);syncCreateCardUi();if(window.lucide)try{lucide.createIcons();}catch(_){}return true;
  }
  ...
  function syncCreateCardUi(){
    ...
    document.querySelectorAll('.agency-card-ready-actions [type="submit"]').forEach(button=>{const edit=button.dataset.afterSave==='edit';button.innerHTML=edit?(enabled?'<i data-lucide="arrow-right" size="16"></i>Create & edit card':'<i data-lucide="user-round-plus" size="16"></i>Add client'):(enabled?'<i data-lucide="users-round" size="16"></i>Create & add another':'<i data-lucide="users-round" size="16"></i>Add & add another');});
  ...
  async function submitCardReadyClient(event){
    ...
    try{
      ...
      let card=null;if(createCard)card=await createDraftCard(client,identity,selectedTemplate());needsRefresh=true;
  ...
  async function prepare(){
    ...
    readyPromise=(async()=>{await resolveOwner();await resolveFlowAccess();await loadTemplates();installForm();})().catch(error=>{console.error('Agency card-ready workflow could not initialize:',error);installForm();});
    return readyPromise;
  }
